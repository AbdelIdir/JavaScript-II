...
runners.forEach(runnerName =>
  fullNames.push(
    `First name: ${runnerName.first_name}, Last name: ${runnerName.last_name}`
  )
);
...
let firstNamesAllCaps = runners.map(people => people.first_name.toUpperCase());
...
let runnersLargeSizeShirt = runners.filter(people => people.shirt_size === "L");
...
// pick only the people with M in their name

const peopleWithMInTheirName = runners.filter(people =>
  people.last_name.includes("M")
);

console.log(peopleWithMInTheirName);

// Problem 2

// People with donations higher than 100

const bigSponsors = runners.filter(people => people.donation > 100);

console.log(bigSponsors);

// Problem 3

// take all the first names of the runners, change them to lower case, and tthen give each of them its own array. Store all those array within one array.

const alphabeticalOrderFirstNames = runners.map(people => {
  return people.first_name.toLowerCase().split();
});

console.log(alphabeticalOrderFirstNames);
